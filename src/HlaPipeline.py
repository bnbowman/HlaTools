...
from pbhla.annotation.separate_amplicons import separate_amplicons
...
class HlaPipeline( object ):
    ...
    def run( self ):
        ...
        self.separate_subreads_by_amplicon()
    ...
    def assemble_contigs( self ):
        ...
        check_output_file( contig_output )
    ...
    def separate_subreads_by_contig(self):
        ...
        separated_seqs = separate_sequences( self.hla_subreads, 
                                             self.subread_contig_dict )
        subread_prefix = os.path.join(self.subreads, 'Contig')
        ...
        write_list_file( self.contig_subread_files, contig_subread_fofn )
        log.info('Finished separating subreads by contig')

    def separate_subreads_by_locus(self):
        log.info("Separating subreads by aligned Locus")
        self.locus_subread_fofn = os.path.join( self.subreads, "Locus_Subread_Files.fofn" )
        if valid_file( self.locus_subread_fofn ):
            log.info('Found existing Locus-Subread File List "{0}"'.format(self.locus_subread_fofn))
            log.info("Skipping subread separation step...\n")
            self.locus_subread_files = read_list_file( self.locus_subread_fofn )
            return
        separated_seqs = separate_sequences( self.hla_subreads, 
                                             self.subread_locus_dict )
        subread_prefix = os.path.join(self.subreads, 'Locus')
        subread_files = write_all_groups( separated_seqs, subread_prefix )
        self.locus_subread_files = [fn for fn in subread_files if not fn.endswith('Unmapped.fasta')]
        write_list_file( self.locus_subread_files, self.locus_subread_fofn )
        log.info('Finished separating subreads by locus')

    def separate_subreads_by_amplicon(self):
        log.info("Separating subreads by probable amplicon location")
        for locus in args.split:
            separate_amplicons( self.locus_subread_fofn, args.reference_file, locus )
        log.info('Finished separating subreads by amplicon')
    ...
    def phase_loci_with_amp_assem( self ):
        ...
        for subread_file in self.renamed_locus_subread_files:
            ...
            if parts[2] in ['3p', '5p']:
                locus_name = '%s_%s-%s' % (parts[0], parts[1], parts[2])
            else:
