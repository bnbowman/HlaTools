...
ENTROPY_TH = - P_THRESHOLD * log(P_THRESHOLD) - (1-P_THRESHOLD) * log( 1-P_THRESHOLD)
...
def read_node_vector(g, entropy_th = 0.65):
...
def partition_reads(read_vector, g_id):
    ...
    if len(group1) < MIN_GROUP_SIZE or len(group2) < MIN_GROUP_SIZE:
        ...
        rtn.extend( partition_reads(group1, g_id * 2 + 1 ) )
        rtn.extend( partition_reads(group2, g_id * 2 + 2 ) )
    
    return rtn
    


def level2_partition(read_ids, read_file, ref_file):
    ...
    rv, hen = read_node_vector(aln_g, entropy_th = ENTROPY_TH)
    if len(rv) == 0:
    ...
    read_groups = partition_reads(aln_data, 0)
    ...
    for rg in read_groups:
        ...
        group_id += 1
        level2_group.extend( level2_partition(r_ids, read_out_file, tmp_cns, ) )
    return level2_group

if __name__ == "__main__":

    read_f = FastaReader( READ_FILE )
    ...
    cns = os.path.join( WORK_DIR, "group_root_cns.fa")
    ...
    score = os.path.join( WORK_DIR, "group_root.score")
    ...
    for r in read_f:
        r_ids.add(r.name)
    level2_group = level2_partition(r_ids, READ_FILE, REF_FILE)
    s = 0
    print "-------------------"
    group_id = 1

    summary_f = open(os.path.join( WORK_DIR, "summary.txt" ), "w")

    for id_set, seq, c_data, status in level2_group:
        out_read_file =  os.path.join( WORK_DIR, "group_%02d.fa" % group_id )
        fetch_read(READ_FILE, out_read_file, id_set)
        cns = os.path.join( WORK_DIR, "group_%02d_cns.fa" % group_id )
        ...
        with open(cns,"w") as f:
            print >>f, ">%s_group_%02d_cns" % (prefix, group_id)
            print >>f, seq
            
        score = c_data

        score = os.path.join( WORK_DIR, "group_%02d.score" % group_id )
        with open(score,"w") as f:
            for i in range(len(seq)):
                print >>f, i, seq[i], " ".join([str(c) for c in c_data[i]]), 1.0*c_data[i][0]/(c_data[i][3]+1)
